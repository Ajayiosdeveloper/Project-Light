...
extension JSQSystemSoundPlayer {
    ...
    private class func jsq_playSoundFromJSQMessagesBundle(name name: String, asAlert: Bool) {
        ...
        if asAlert {
                     ...
                     JSQSystemSoundPlayer.sharedPlayer().playAlertSoundWithFilename(filename, fileExtension:"aiff", completion:nil)
            
        }
        else {
            
            JSQSystemSoundPlayer.sharedPlayer().playSoundWithFilename(filename, fileExtension: "aiff", completion: nil)
